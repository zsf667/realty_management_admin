  ...
  <div>
    <el-card class="box-card">
      ...
      <div slot="header" class="clearfix">
        <span style="font-size: 20px">保修管理</span>
      </div>
      <!-- 搜索 -->
      <el-form ref="form" :model="form" label-width="65px" :inline="true">
        <el-form-item label="申请人">
          <el-input
            ...
            placeholder="申请人"
            @input="searchWarranty"
          ></el-input>
        </el-form-item>
        <el-form-item>
          <el-button type="primary" @click="searchWarranty">搜索</el-button>
        </el-form-item>
      </el-form>
      <!-- 列表 -->
...
export default {
  ...
  methods: {
    // 获取报修的数据
    async getWarranty(data) {
      const res = await this.$API.guarantee.getWarranty(data);
      if (res.code == 1) {
    ...
    async confirm() {
      ...
      const res = await this.$API.guarantee.updateRepair(data);
      if (res.code == 1) {
